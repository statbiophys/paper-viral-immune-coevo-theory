 ...
 /* adapted by Jacopo Marchi in 2021 from the original NFFT code documented here: https://www-user.tu-chemnitz.de/~potts/nfft/fastsum.php
  * 
  * 
  * 
...
//
//
//std::complex<double> fastsum_plan_::k(R x, const R *param) // distance and scale param in renormalized units
//{
//  R c = param[0];
//  R value = 0.;
//
//    value = exp(-FABS(x)/c); 
//    
//    //std::cout << c << " " << x << " " << FABS(x)  << " " << value << std::endl;
//
//  return value;
//}
      ...
      sum +=
          POW(a, (R) r)
              * (k(-a, r, param) * BasisPoly(p - 1, r, xx / a)
                  + k(a, r, param) * BasisPoly(p - 1, r, -xx / a)
                      * *reinterpret_cast<std::complex<double>*>((r & 1 ? -1 : 1)));
    }
    return sum;
  }
  else if (xx > K(0.5) - b)
 ...
 *  (used in 1D)
 */
static std::complex<double>  regkern1(kernel k, R xx, int p, const R *param, R a, R b)
{
  int r;
  std::complex<double>  sum = K(0.0);
...
//static std::complex<double>  regkern2(kernel k, R xx, int p, const R *param, R a, R b)
//{
//  int r;
//  std::complex<double>  sum = K(0.0);
//
 ...
 */
static std::complex<double>  regkern3(kernel k, R xx, int p, const R *param, R a, R b)
{
  int r;
  std::complex<double>  sum = K(0.0);
...
//static std::complex<double>  linintkern(const R x, const std::complex<double>  *Add, const int Ad, const R a)
//{
//  R c, c1, c3;
//  int r;
...
//}
//
//static std::complex<double>  quadrintkern(const R x, const std::complex<double>  *Add, const int Ad, const R a)
//{
//  R c, c1, c2, c3;
//  int r;
...
std::complex<double>  kubintkern(const R x, const std::complex<double>  *Add, const int Ad, const R a)
{
  R c, c1, c2, c3, c4;
  int r;
  std::complex<double>  f0, f1, f2, f3;
  c = x * (R)(Ad) / a;
  r = (int)(LRINT(c));
  r = abs(r);
  if (r == 0)
...
static std::complex<double>  kubintkern1(const R x, const std::complex<double>  *Add, const int Ad, const R a)
{
  R c, c1, c2, c3, c4;
  int r;
  std::complex<double>  f0, f1, f2, f3;
...
static void quicksort(int d, int t, R *x, std::complex<double>  *alpha, int *permutation_x_alpha, int N)
{
  int lpos = 0;
  int rpos = N - 1;
  /*R pivot=x[((N-1)/2)*d+t];*/
...
static inline std::complex<double>  calc_SearchBox(int d, R *y, R *x, std::complex<double>  *alpha, int start,
    int end_lt, const std::complex<double>  *Add, const int Ad, int p, R a, const kernel k,
    const R *param, const unsigned flags)
{
  std::complex<double>  result = K(0.0);
...
static std::complex<double>  SearchBox(R *y, fastsum_plan *ths)
{
  std::complex<double>  val = K(0.0);
  int t;
...
static void BuildTree(int d, int t, R *x, std::complex<double>  *alpha, int *permutation_x_alpha, int N)
{
  if (N > 1)
  {
    int m = N / 2;
...
static std::complex<double>  SearchTree(const int d, const int t, const R *x, const std::complex<double>  *alpha,
    const R *xmin, const R *xmax, const int N, const kernel k, const R *param,
    const int Ad, const std::complex<double>  *Add, const int p, const unsigned flags)
{
  if (N == 0)
  {
      return K(0.0);
...
std::complex<double> kern_nfft_exp(R x, int der, const R *param)    /* K(x)=EXP(-|x|/c) */
{
  R c = param[0];
  R value = K(0.0);
...
//#endif
  for (j = 0; j < n_total; j++)
  {
    ...
    else
    {
      ...
      ths->b[j] = regkern3(ths->k, SQRT((ths->b[j]).real()), ths->p, ths->kernel_param,
  ...
  NFFT(fftshift_complex)(reinterpret_cast<fftw_complex*>(ths->b), (int)(ths->d), N);
...
void fastsum_init_guru_kernel(fastsum_plan *ths, int d, kernel k, R *param,
    unsigned flags, int nn, int p, double eps_I, double eps_B)
{
  int t;
  int N[d];
  int n_total;
...
//#endif
  ...
  if (ths->eps_I > 0.0 && !(ths->flags & EXACT_NEARFIELD))
  {
    if (ths->d == 1)
    {
      ...
      ths->Add = (std::complex<double> *) NFFT(malloc)((size_t)(ths->Ad + 5) * (sizeof(std::complex<double>)));
    }
    else
    {
//      if (ths->k == one_over_x)
...
//        ths->Add = (std::complex<double> *) NFFT(malloc)((size_t)(ths->Ad + 3) * (sizeof(std::complex<double>)));
//#elif defined(NF_QUADR)
...
//      }
  ...
  ths->b = (std::complex<double>*) NFFT(malloc)((size_t)(n_total) * sizeof(std::complex<double>));
  ths->f_hat = (std::complex<double>*) NFFT(malloc)((size_t)(n_total) * sizeof(std::complex<double>));
//#ifdef _OPENMP
...
//#endif
  ...
  ths->fft_plan = FFTW(plan_dft)(d, N, reinterpret_cast<fftw_complex*>(ths->b), reinterpret_cast<fftw_complex*>(ths->b), FFTW_FORWARD,
      FFTW_ESTIMATE);
...
//#endif
  ...
  ths->alpha = (std::complex<double> *) NFFT(malloc)((size_t)(N_total) * (sizeof(std::complex<double>)));
  ...
  ths->mv1.x = ths->x;
  ths->mv1.f = reinterpret_cast<fftw_complex*>(ths->alpha);
  ths->mv1.f_hat = reinterpret_cast<fftw_complex*>(ths->f_hat);
  ...
  if (ths->flags & NEARFIELD_BOXES)
  {
    if (ths->eps_I > 0.0)
    {
      ...
      ths->box_alpha = (std::complex<double> *) NFFT(malloc)((size_t)(ths->N_total) * (sizeof(std::complex<double>)));
...
void fastsum_init_guru_target_nodes(fastsum_plan *ths, int M_total, int nn_oversampled, int m)
{
  ...
  ths->f = (std::complex<double> *) NFFT(malloc)((size_t)(M_total) * (sizeof(std::complex<double>)));
  ...
  NFFT(init_guru)(&(ths->mv2), ths->d, N, M_total, n, m,
      ...
      FFTW_ESTIMATE | FFTW_DESTROY_INPUT);
  ths->mv2.x = ths->y;
  ths->mv2.f = reinterpret_cast<fftw_complex*>(ths->f);
  ths->mv2.f_hat = reinterpret_cast<fftw_complex*>(ths->f_hat);
}
...
void fastsum_init_guru(fastsum_plan *ths, int d, int N_total, int M_total,
    kernel k, R *param, unsigned flags, int nn, int m, int p, double eps_I, double eps_B)
{
...
//#endif
  for (j = 0; j < ths->M_total; j++)
  {
    ...
    for (k = 0; k < ths->N_total; k++)
    {
      ...
      //std::cout << j << " " << k << " " << ths->f[j]  << " " << ths->alpha[k]  << " " << r << " " << ths->k(r, ths->kernel_param) << std::endl;
...
//#endif
  for (k = 0; k < ths->mv2.N_total; k++){
    //reinterpret_cast<double*>(ths->mv2.f_hat)[k] =  reinterpret_cast<double*>(ths->b)[k] * reinterpret_cast<double*>(ths->mv1.f_hat)[k];
    //ths->mv2.f_hat[k] =  reinterpret_cast<double*>(*(reinterpret_cast<std::complex<double>*>(ths->b[k]) * ths->mv1.f_hat[k]));
    
    std::complex<double>  temp_comp= (*reinterpret_cast<std::complex<double> *>(&(ths->b[k])) * *reinterpret_cast<std::complex<double> *>(ths->mv1.f_hat[k]));
    //fftw_complex * temp_comp_cast= reinterpret_cast<fftw_complex*>(&temp_comp);
    //std::complex<double> temp_comp= (ths->b[k] * *reinterpret_cast<std::complex<double>*>(ths->mv1.f_hat[k]));
    //fftw_complex * temp_comp_cast= reinterpret_cast<fftw_complex*>(&temp_comp);
    
    reinterpret_cast<std::complex<double> *>(ths->mv2.f_hat)[k] =  temp_comp;
...
//#endif
  for (j = 0; j < ths->M_total; j++)
    ths->f[j] = *reinterpret_cast<std::complex<double>*>(ths->mv2.f[j]);
